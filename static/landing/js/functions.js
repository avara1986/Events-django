...
function sendAttendee(){
	var _data = getFields('formAttendees');
	$('#formErrorGeneric').css('display','none');
    if(_data!=null){
    	$('#formAttendees').submit();
    }else{
    	$('#formErrorGeneric').css('display','block');
    }
    return false;
}
$('#sendAttendee').click(function() {
	sendAttendee();
});
/*
function sendAttendee(){
    ...
    if(_data!=null){
    	...
    	.success(function(data) {
    		...
    		if(data.response==true) {
    			window.location.replace(window.location.href+'?result=ok');
    			// TODO
